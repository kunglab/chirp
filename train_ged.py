...
from dragan.updater import GEDUpdater 
...
report_keys = ['loss_dis', 'loss_gen', 'loss_gen_c', 'loss_dis_c', 'loss_noise', 'loss_gp', 'loss_enc', 'loss_e_recon', 'loss_e_noise']
...
model_names = ['DCGANGenerator', 'LabeledDiscriminatorJoined', 'EncoderLabeledDiscriminatorJoined']
models = []
...
updater = GEDUpdater(**{
    ...
    'optimizer': {
        ...
        'opt_enc': make_optimizer(encoder, args.adam_alpha, args.adam_beta1, args.adam_beta2),
    },
...
for m_name, m in zip(model_names, models):
    trainer.extend(extensions.snapshot_object(
        m, m_name + '_{.updater.iteration}.npz'), trigger=(args.snapshot_interval, 'iteration'))
...
trainer.extend(encdis_generate(generator, encoder, results_output_dir, train_max=train_max),
               trigger=(args.evaluation_interval, 'iteration'),
               priority=extension.PRIORITY_WRITER)
trainer.extend(encdis_generate_light(generator, encoder, results_output_dir, train_max=train_max),
